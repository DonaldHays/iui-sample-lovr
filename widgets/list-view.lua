...
function iui.listView(name, count, rowHeight, itemBuilder, manager)
    ...
    iui.scrollView("ScrollView", function()
        ...
        panel.contentHeight = math.max(
            panel.contentHeight, count * rowHeight - manager.y
        )

        local minIndex = math.max(math.floor(manager.y / rowHeight) + 1, 1)
        local maxIndex = math.min(
            math.ceil((manager.y + panel.h) / rowHeight), count
        )

        for index = minIndex, maxIndex do
            panel.rowY = (index - 1) * rowHeight - manager.y
